...
assessments_train = df_train[(df_train['type']=='Assessment') & 
...
assessments_train.loc[:,'Success'] = assessments_train['event_data'].apply(lambda x: json.loads(x)['correct'])
assessments_train = pd.get_dummies(assessments_train, columns=['Success'], dummy_na=False)
assessments_train.loc[:,'Success_False'] = assessments_train.Success_False.astype(int)
assessments_train.loc[:,'Success_True'] = assessments_train.Success_True.astype(int)
...
# Construct df_labels
train_labels = assessments_train.groupby(['game_session','installation_id','title']).agg({
                                                                        'Success_False':'sum',
...
last_assessment_train = assessments_train[assessments_train.groupby(['installation_id'])['new_timestamp'].transform(max) == assessments_train['new_timestamp']]
